...
void Level::moveObjects(Player *player, Hud &hud){
  ...
  if (shadow.comparePos(shadow.position,player->position)) { playerContact(player,shadow, hud); }
  if (speedy.comparePos(speedy.position,player->position)) { playerContact(player,speedy, hud); }
  if (bashful.comparePos(bashful.position,player->position)) { playerContact(player,bashful, hud); }
  if (pokey.comparePos(pokey.position,player->position)) { playerContact(player,pokey, hud); }
...
void Level::playerContact(Player *player, Ghost ghost, Hud &hud){
  if (statePacman == pacmanHunted){
    player->takeDamage(hud);
